...
import {
  API_USER_COURSE_COMPLETE_LESSON,
  API_USER_COURSE_VIEW,
...
const UserCourseView = () => {
  ...
  const getCourses = () => {
    setLoading(true);
    getRequest(API_USER_COURSE_VIEW + "/" + param.id)
      .then((res) => {
        setCourse(res.data);
        setLoading(false);
        setValidUser(true);
      ...
      .catch((err) => {
        ...
        setLoading(false);
        message.error(
          "You have not enrolled the course. Kindly enroll the coures to access the course!!"
        );
        history.push("/");
      });
  };
              ...
              {clicked !== -1 ? (
                <Fragment>
                  <div className="col alert alert-primary square">
                    <b>
                      {course.courseLeasons[clicked].title.substring(0, 30)}
                    </b>
                    {completedLessons.includes(
                      course.courseLeasons[clicked].id
                    ) ? (
                      <span
                  ...
                  <p>{course.courseLeasons[clicked].content}</p>
                </Fragment>
              ) : (
