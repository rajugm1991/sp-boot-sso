...
import {
  API_COURSE_GET_URL,
  API_COURSE_PAID_SUBSCRIBE,
  API_COURSE_SUBSCRIBE,
  STRIPE_PUBLIC_KEY,
...
const SingleCourse = () => {
  ...
  useEffect(() => {
    ...
    if (Object.keys(courseData).length > 0) {
      getRequest(API_COURSE_SUBSCRIBE + "/" + courseData.id)
        .then((res) => {
          setSubData(res.data);
          setLoading(false);
          setTemp(true);
  ...
  useEffect(() => {
    ...
    getRequest(API_COURSE_GET_URL + "/" + location.state.id)
      ...
      .catch((err) => {
        console.log(
          "SINGLE_COURSE_VIEW: error while getting course details " + err
        );
        setLoading(false);
      });
  }, [location.state.id]);
    ...
    <Fragment>
            ...
            <div className="jumbotron bg-primary square">
              <div className="row">
                ...
                <div className="col-md-4" style={{ float: "right" }}>
                  {courseData.courseLeasons &&
                  courseData.courseLeasons[0] &&
                  courseData.courseLeasons[0].courseVideo ? (
                    <div>
                      {
                        <ReactPlayer
                  ...
                  {loading ? (
                    ...
                    <Button
                      ...
                      onClick={
                        !temp
                          ? courseData.paid
                            ? () => handlePaidEnrollment(courseData.id)
                            : () => handleFreeEnrollment(courseData.id)
                          : () => {
                              rediretCoursePage(courseData.id);
                            }
                      }
                    >
                      {user
      ...
      <div className="row pb-5">
        <LessonList
          course={courseData}
          onDeleteLesson={() => {
            Promise.reject("You doesn't have rights");
          }}
